...
logger = logging.getLogger("shelly_sensors")
...
class ShellySensorsPlugin(ToolVerba):
    name = "shelly_sensors"
    verba_name = "Shelly Sensors"
    pretty_name = "Shelly Sensors"
    version = "1.0.0"
    ...
    tags = ["shelly", "sensor", "binary-sensor", "temperature", "humidity", "power", "energy"]
    routing_keywords = [
        "sensor",
        "temperature",
        "humidity",
    ...
    usage = '{"function":"shelly_sensors","arguments":{"query":"what is the garage temperature?"}}'
    description = "Read sensor states such as input/contact, temperature, humidity, illuminance, battery, power, and energy."
    verba_dec = "Read sensor and meter states."
    when_to_use = (
        "Use for read-only sensor questions: temperature, humidity, illuminance, battery, input/contact state, "
        "power meter, or energy readings."
    )
    how_to_use = "Pass one natural-language sensor request in query. The verba resolves the sensor resource."
    common_needs = ["Sensor target to read, such as garage temperature or workbench power meter."]
    missing_info_prompts = ["Which sensor should I check?"]
    example_calls = [
        '{"function":"shelly_sensors","arguments":{"query":"what is the garage temperature?"}}',
        '{"function":"shelly_sensors","arguments":{"query":"is the contact sensor open?"}}',
        '{"function":"shelly_sensors","arguments":{"query":"show sensors"}}',
    ]
    ...
    device_types = {"binary_sensor", "temperature", "humidity", "illuminance", "power_meter", "battery", "voltmeter"}
    domain_label = "sensor"
    domain_label_plural = "sensors"
    ...
    async def _ai_interpret(self, llm_client, query: str) -> dict:
        ...
        system = (
            ...
            "Rules:\n"
            "- Use status for read-only sensor, input, contact, temperature, humidity, illuminance, battery, power, or energy questions.\n"
    ...
    async def _ai_choose_device(self, *, query: str, intent: dict, candidates: List[dict], llm_client) -> str:
        ...
        system = (
            "Choose the best sensor for this request.\n"
