...
logger = logging.getLogger("moltbook_monitoring_submolts")
...
class MoltbookMonitoringSubmoltsPlugin(ToolVerba):
    name = "moltbook_monitoring_submolts"
    verba_name = "Moltbook Monitoring Submolts"
    version = "1.0.2"
    min_tater_version = "59"
    pretty_name = "Moltbook Monitoring Submolts"
    settings_category = "Moltbook Info"
    tags = ['moltbook', 'monitoring_submolts']
    fixed_route = "monitoring_submolts"
    platforms = ['webui', 'macos', 'voice_core', 'homeassistant', 'homekit', 'xbmc', 'discord', 'telegram', 'matrix', 'irc', 'meshtastic']

    usage = (
        "{\"function\":\"moltbook_monitoring_submolts\",\"arguments\":{\"query\":\"what submolts are you monitoring\"}}"
    )
    description = (
        "Show monitored/preferred/avoided Moltbook submolt configuration."
    )
    verba_dec = "Read Moltbook portal settings for submolts to monitor/prefer/avoid."
    when_to_use = (
        "Use when the user asks what submolts are monitored, preferred, or avoided."
    )
    how_to_use = (
        "Set `query` to a monitoring-config request (for example: what submolts are you monitoring)."
    )
    common_needs = ['A monitoring-submolts request in query.']
    routing_keywords = ['moltbook', 'monitoring', 'submolts', 'preferred', 'avoided']
    missing_info_prompts = []
    example_calls = ['{"function":"moltbook_monitoring_submolts","arguments":{"query":"what submolts are you monitoring"}}', '{"function":"moltbook_monitoring_submolts","arguments":{"query":"show monitored and avoided submolts"}}']
    argument_schema = {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'The monitoring request (for example: show monitored submolts).'}}, 'required': []}
...
verba = MoltbookMonitoringSubmoltsPlugin()
