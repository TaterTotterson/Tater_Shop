...
logger = logging.getLogger("unifi_protect_sensors")
...
class UniFiProtectSensorsPlugin(ToolVerba):
    ...
    routing_keywords = ['unifi protect', 'protect', 'sensor', 'sensors', 'door sensor', 'window sensor', 'battery', 'humidity', 'temperature', 'temp']
    fixed_action = 'sensors_status'

    usage = '{"function":"unifi_protect_sensors","arguments":{"query":"are any doors open"}}'

    description = 'Check UniFi Protect door/window sensor status, battery, temperature, and humidity.'
    verba_dec = 'Check UniFi Protect sensor status and single-sensor detail.'
    when_to_use = 'Use for sensor status questions or specific sensor checks.'
    how_to_use = 'Set query to the sensor question you want answered, and optionally include target for one named sensor.'
    common_needs = ['Sensor question in query (for example: are any doors open).']
    missing_info_prompts = []
    example_calls = ['{"function":"unifi_protect_sensors","arguments":{"query":"are any doors open"}}', '{"function":"unifi_protect_sensors","arguments":{"query":"any low battery sensors"}}', '{"function":"unifi_protect_sensors","arguments":{"query":"is the garage door sensor open"}}']

    argument_schema = {'type': 'object', 'properties': {'target': {'type': 'string', 'description': 'Optional sensor name (for example: front door, garage door).'}, 'query': {'type': 'string', 'description': 'The sensor question to run (for example: are any doors open, any low battery sensors, is the garage door sensor open).'}}, 'required': []}
    ...
    def _get_client(self) -> Optional[ProtectClient]:
        try:
            ...
            logger.error(f"[unifi_protect_sensors] Failed to init client: {e}")
            return None
    ...
    async def _handle(self, args, llm_client, platform: str = "webui", context: Optional[dict] = None):
        ...
        if not client:
            return action_failure(
                code="unifi_protect_sensors_not_configured",
                message=(
        ...
        if action in ("sensors_status", "sensor_detail"):
            try:
                ...
                logger.error(f"[unifi_protect_sensors] list_sensors error: {e}")
                return action_failure(
        ...
        if action == "list_cameras":
            try:
                ...
                logger.error(f"[unifi_protect_sensors] list_cameras error: {e}")
                return action_failure(
        ...
        if action == "describe_camera":
            ...
            try:
                ...
                logger.error(f"[unifi_protect_sensors] snapshot error: {e}")
        ...
        if action == "describe_area":
            ...
            for cam in picked:
                ...
                try:
                    ...
                    logger.info(f"[unifi_protect_sensors] snapshot/vision failed for {cam_name}: {e}")
                    camera_descriptions.append({
                        "camera": cam_name,
                        "state": cam.get("state"),
...
verba = UniFiProtectSensorsPlugin()
