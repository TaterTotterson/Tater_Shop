...
class HACoversPlugin(ToolVerba):
    ...
    how_to_use = 'Pass a natural-language query including the cover/door and the action.'
    common_needs = ['Cover/device and action (for example: garage door + open).']
    missing_info_prompts = ['Which cover or garage door should I control?']
    example_calls = ['{"function":"ha_covers","arguments":{"query":"open the garage door"}}', '{"function":"ha_covers","arguments":{"query":"is the patio shade closed"}}']

    entity_domains: List[str] = ['cover']
    interpret_actions: List[str] = ['get_state', 'open', 'close', 'stop']
    service_map: Dict[str, Optional[str]] = {'get_state': None, 'open': 'open_cover', 'close': 'close_cover', 'stop': 'stop_cover'}
    required_action_params: Dict[str, List[str]] = {}
    optional_action_params: Dict[str, List[str]] = {}
    param_payload_keys: Dict[str, str] = {}
    summary_attribute_keys: List[str] = ['current_position']
    interpret_focus: str = '- Use open/close/stop for cover movement actions.\n- Use get_state for status checks.'
    interpret_examples: List[str] = ['open the garage door -> action=open, target=garage door, read_target=none', 'close patio shade -> action=close, target=patio shade, read_target=none', 'is the garage door open -> action=get_state, target=garage door, read_target=state']
