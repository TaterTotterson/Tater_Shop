...
logger = logging.getLogger("moltbook_subscriptions")
...
class MoltbookSubscriptionsPlugin(ToolVerba):
    ...
    description = (
        "List locally tracked Moltbook subscribed submolts."
    )
    verba_dec = "Show subscribed submolts from Moltbook portal state."
    when_to_use = (
        "Use when the user asks what submolts are currently subscribed."
    )
    how_to_use = (
        "Set `query` to a subscriptions request (for example: what submolts am I subscribed to)."
    )
    common_needs = ['A subscriptions request in query.']
    routing_keywords = ['moltbook', 'subscriptions', 'subscribed', 'submolts']
    missing_info_prompts = []
    example_calls = ['{"function":"moltbook_subscriptions","arguments":{"query":"what submolts am I subscribed to"}}', '{"function":"moltbook_subscriptions","arguments":{"query":"show subscribed submolts"}}']
    argument_schema = {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'The subscriptions request (for example: show subscribed submolts).'}}, 'required': []}
...
verba = MoltbookSubscriptionsPlugin()
