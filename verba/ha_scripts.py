...
logger = logging.getLogger("ha_scripts")
...
class HAScriptsPlugin(ToolVerba):
    ...
    def _get_client(self):
        try:
            ...
            logger.error(f"[ha_scripts] Failed to initialize HA client: {e}")
            return None
    ...
    def _excluded_entities_set(self) -> set[str]:
        ...
        for k in keys:
            ...
            if v:
                ids.append(v.lower())
        excluded = set(ids)
        logger.debug(f"[ha_scripts] excluded voice PE entities: {excluded}")
        return excluded
    ...
    async def _base_interpret_query(self, query: str, llm_client) -> dict:
        ...
        cache_key = f"ha_scripts:interpret:v1:{query.strip().lower()}"
        if cache_ttl > 0:
    ...
    async def _choose_entity_llm(
        ...
        *,
        ...
        cache_key = None
        if cache_ttl > 0:
            cache_key = f"ha_scripts:choose:v3:{(query or '').strip().lower()}:{str(cat_ts or 'none')}:{_json.dumps(intent, sort_keys=True, ensure_ascii=False)}"
            try:
                cached = self._cache_get_json(cache_key)
        ...
        if len(mini) <= max_single:
            try:
                ...
                logger.warning(f"[ha_scripts] LLM choose failed single-shot: {e}")
        ...
        if not picked:
            try:
                ...
                logger.warning(f"[ha_scripts] LLM choose failed tournament: {e}")
                picked = mini[0]["entity_id"] if mini else None
    ...
    async def _base_handle(self, args, llm_client):
        ...
        try:
            ...
            logger.error(f"[ha_scripts] catalog build failed: {e}")
            return "I couldn't access Home Assistant states."

        try:
            intent = await self._interpret_query(query, llm_client)
        except Exception as e:
            logger.error(f"[ha_scripts] interpret_query failed: {e}")
        ...
        if intent_type == "get_temp" or primary_route == "temperature":
            ...
            try:
                ...
                logger.error(f"[ha_scripts] temp get_state error: {e}")
        ...
        if primary_route == "climate" and intent_type in ("get_state", "control") and action == "get_state":
            ...
            try:
                ...
                logger.error(f"[ha_scripts] thermostat read error: {e}")
                return f"Error reading {entity_id}: {e}"
        ...
        if intent_type == "set_temperature" or action == "set_temperature":
            ...
            try:
                ...
                logger.error(f"[ha_scripts] set_temperature error: {e}")
        ...
        if intent_type == "control":
            ...
            if entity_domain == "remote":
                ...
                try:
                    if service == "turn_on":
                        ...
                        try:
                            ...
                            logger.info(f"[ha_scripts] remote.turn_on failed: {e}")
                        ...
                        if not did_work:
                            ...
                            if mp_eid:
                                try:
                                    ...
                                    logger.info(f"[ha_scripts] media_player.turn_on fallback failed: {e}")
                        ...
                        try:
                            ...
                            logger.info(f"[ha_scripts] remote.turn_off failed: {e}")
                        ...
                        if not did_work:
                            ...
                            if mp_eid:
                                try:
                                    ...
                                    logger.info(f"[ha_scripts] media_player.turn_off fallback failed: {e}")
                        ...
                        if last_error is not None:
                            logger.info(f"[ha_scripts] remote.send_command failed. Tried: {tried}. Last error: {last_error}")
                            return (
                    ...
                    logger.error(f"[ha_scripts] remote control error: {e}")
                ...
                try:
                    ...
                    logger.error(f"[ha_scripts] remote post-state error: {e}")
                    return "Done."
            ...
            try:
                ...
                logger.error(f"[ha_scripts] control error: {e}")
                return action_failure(
                    code="ha_scripts_failed",
        ...
        if intent_type == "get_state" or action == "get_state":
            ...
            try:
                ...
                logger.error(f"[ha_scripts] get_state error: {e}")
                return f"Error reading {entity_id}: {e}"
