...
class HAScriptsPlugin(ToolVerba):
    ...
    description = 'Run Home Assistant scripts and check script state.'
    verba_dec = 'Control Home Assistant scripts.'
    when_to_use = 'Use for explicit script execution and script state checks.'
    how_to_use = 'Pass a natural-language script request with script name and action.'
    common_needs = ['Script name and action (for example: bedtime script + run).']
    missing_info_prompts = ['Which script should I run?']
    example_calls = ['{"function":"ha_scripts","arguments":{"query":"run bedtime script"}}', '{"function":"ha_scripts","arguments":{"query":"is bedtime script on"}}']

    entity_domains: List[str] = ['script']
    interpret_actions: List[str] = ['get_state', 'run']
    service_map: Dict[str, Optional[str]] = {'get_state': None, 'run': 'turn_on'}
    required_action_params: Dict[str, List[str]] = {}
    optional_action_params: Dict[str, List[str]] = {}
    param_payload_keys: Dict[str, str] = {}
    summary_attribute_keys: List[str] = ['last_triggered']
    interpret_focus: str = '- Use run for explicit script execution requests.\n- Use get_state for status checks.'
    interpret_examples: List[str] = ['run bedtime script -> action=run, target=bedtime script, read_target=none', 'is bedtime script running -> action=get_state, target=bedtime script, read_target=state']
