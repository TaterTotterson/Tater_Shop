# verba/unifi_network_clients.py
import json
import logging
import re
...
class UnifiNetworkClientsPlugin(ToolVerba):
    ...
    verba_dec = (
        "List UniFi clients with wired/wireless counts and client details."
    )
    when_to_use = (
        "Use for UniFi client lists, online-client checks, or wired/wireless client count questions."
    )
    how_to_use = (
        "Set query to the client request you want answered (for example: list clients, who is online, how many wireless clients)."
    )
    example_calls = [
        '{"function":"unifi_network_clients","arguments":{"query":"list clients"}}',
        '{"function":"unifi_network_clients","arguments":{"query":"who is online"}}',
        '{"function":"unifi_network_clients","arguments":{"query":"how many wireless clients"}}',
    ]
    common_needs = ["Client question in query."]
    routing_keywords = [
        "unifi",
        "unifi network",
        "wifi",
    ...
    argument_schema = {
        ...
        "properties": {
            "query": {
                "type": "string",
                "description": "Client request (for example: list clients, who is online, how many wireless clients).",
            }
        },
        "required": [],
    }
...
verba = UnifiNetworkClientsPlugin()
