...
logger = logging.getLogger("moltbook_following")
...
class MoltbookFollowingPlugin(ToolVerba):
    name = "moltbook_following"
    verba_name = "Moltbook Following"
    version = "1.0.0"
    min_tater_version = "59"
    pretty_name = "Moltbook Following"
    settings_category = "Moltbook Info"
    tags = ['moltbook', 'following']
    fixed_route = "following"
    platforms = ['webui', 'macos', 'homeassistant', 'homekit', 'xbmc', 'discord', 'telegram', 'matrix', 'irc']

    usage = (
        "{\"function\":\"moltbook_following\",\"arguments\":{\"query\":\"who are you following on Moltbook\"}}"
    )
    description = (
        "Show who this Moltbook agent follows."
    )
    verba_dec = "List known followed agents and active authors from following feed."
    when_to_use = (
        "Use when the user asks who you are following on Moltbook."
    )
    how_to_use = (
        "Set `query` to a following request and optionally set `limit`."
    )
    common_needs = ['A Moltbook following request in query.']
    routing_keywords = ['moltbook', 'following', 'follow list', 'who do you follow']
    missing_info_prompts = []
    example_calls = ['{"function":"moltbook_following","arguments":{"query":"who are you following on Moltbook"}}', '{"function":"moltbook_following","arguments":{"query":"show your following list","limit":10}}']
    argument_schema = {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'The following request (for example: who are you following).'}, 'limit': {'type': 'integer', 'description': 'Optional display/sample limit hint (1-20).'}}, 'required': []}
