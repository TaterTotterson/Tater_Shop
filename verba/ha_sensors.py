...
class HASensorsPlugin(ToolVerba):
    ...
    def _get_client(self):
        try:
            ...
            logger.error(f"[ha_sensors] Failed to initialize HA client: {e}")
            return None
    ...
    def _excluded_entities_set(self) -> set[str]:
        ...
        for k in keys:
            ...
            if v:
                ids.append(v.lower())
        excluded = set(ids)
        logger.debug(f"[ha_sensors] excluded voice PE entities: {excluded}")
        return excluded
    ...
    async def _base_interpret_query(self, query: str, llm_client) -> dict:
        ...
        cache_key = f"ha_sensors:interpret:v1:{query.strip().lower()}"
        if cache_ttl > 0:
    ...
    async def _choose_entity_llm(
        ...
        *,
        ...
        cache_key = None
        if cache_ttl > 0:
            cache_key = f"ha_sensors:choose:v3:{(query or '').strip().lower()}:{str(cat_ts or 'none')}:{_json.dumps(intent, sort_keys=True, ensure_ascii=False)}"
            try:
                cached = self._cache_get_json(cache_key)
        ...
        if len(mini) <= max_single:
            try:
                ...
                logger.warning(f"[ha_sensors] LLM choose failed single-shot: {e}")
        ...
        if not picked:
            try:
                ...
                logger.warning(f"[ha_sensors] LLM choose failed tournament: {e}")
                picked = mini[0]["entity_id"] if mini else None
    ...
    async def _base_handle(self, args, llm_client):
        ...
        try:
            ...
            logger.error(f"[ha_sensors] catalog build failed: {e}")
            return "I couldn't access Home Assistant states."

        try:
            intent = await self._interpret_query(query, llm_client)
        except Exception as e:
            logger.error(f"[ha_sensors] interpret_query failed: {e}")
        ...
        if intent_type == "get_temp" or primary_route == "temperature":
            ...
            try:
                ...
                logger.error(f"[ha_sensors] temp get_state error: {e}")
        ...
        if primary_route == "climate" and intent_type in ("get_state", "control") and action == "get_state":
            ...
            try:
                ...
                logger.error(f"[ha_sensors] thermostat read error: {e}")
                return f"Error reading {entity_id}: {e}"
        ...
        if intent_type == "set_temperature" or action == "set_temperature":
            ...
            try:
                ...
                logger.error(f"[ha_sensors] set_temperature error: {e}")
        ...
        if intent_type == "control":
            ...
            if entity_domain == "remote":
                ...
                try:
                    if service == "turn_on":
                        ...
                        try:
                            ...
                            logger.info(f"[ha_sensors] remote.turn_on failed: {e}")
                        ...
                        if not did_work:
                            ...
                            if mp_eid:
                                try:
                                    ...
                                    logger.info(f"[ha_sensors] media_player.turn_on fallback failed: {e}")
                        ...
                        try:
                            ...
                            logger.info(f"[ha_sensors] remote.turn_off failed: {e}")
                        ...
                        if not did_work:
                            ...
                            if mp_eid:
                                try:
                                    ...
                                    logger.info(f"[ha_sensors] media_player.turn_off fallback failed: {e}")
                        ...
                        if last_error is not None:
                            logger.info(f"[ha_sensors] remote.send_command failed. Tried: {tried}. Last error: {last_error}")
                            return (
                    ...
                    logger.error(f"[ha_sensors] remote control error: {e}")
                ...
                try:
                    ...
                    logger.error(f"[ha_sensors] remote post-state error: {e}")
                    return "Done."
            ...
            try:
                ...
                logger.error(f"[ha_sensors] control error: {e}")
                return action_failure(
                    code="ha_sensors_failed",
        ...
        if intent_type == "get_state" or action == "get_state":
            ...
            try:
                ...
                logger.error(f"[ha_sensors] get_state error: {e}")
                return f"Error reading {entity_id}: {e}"
