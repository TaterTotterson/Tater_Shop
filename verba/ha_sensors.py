...
class HASensorsPlugin(ToolVerba):
    ...
    verba_dec = 'Read Home Assistant sensor and binary_sensor entities.'
    when_to_use = 'Use for non-temperature sensor status checks.'
    how_to_use = 'Pass a natural-language query describing which sensor status you want.'
    common_needs = ['Sensor/device to read (for example: front door sensor state).']
    missing_info_prompts = ['Which sensor should I check?']
    example_calls = ['{"function":"ha_sensors","arguments":{"query":"is the back door sensor open"}}', '{"function":"ha_sensors","arguments":{"query":"is motion detected in the garage"}}']
    ...
    summary_attribute_keys: List[str] = ['device_class']
    interpret_focus: str = '- This verba is read-only.\n- Use get_state only.\n- Focus on contact/motion/occupancy and other non-temperature sensors.'
    interpret_examples: List[str] = ['is the back door sensor open -> action=get_state, target=back door sensor, read_target=state', 'is motion detected in garage -> action=get_state, target=garage motion sensor, read_target=state']

    domain_read_only: bool = True
    include_binary_sensor: bool = True
    temperature_only: bool = False
    exclude_temperature: bool = True
