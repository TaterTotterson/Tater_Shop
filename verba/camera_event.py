...
class CameraEventPlugin(ToolVerba):
    """
    ...
    def _get_settings(self) -> Dict[str, str]:
        ...
        legacy = (
            redis_client.hgetall("verba_settings:Phone Events Alert")
            or redis_client.hgetall("verba_settings: Phone Events Alert")
            or {}
        )
        if not legacy:
            return settings
    ...
    def _post_event(
        ...
        *,
        ...
        try:
            ...
            return {"ok": True, "via": "redis_direct"}
        except Exception as exc:
            logger.warning("[camera_event] redis event write failed: %s", exc)
