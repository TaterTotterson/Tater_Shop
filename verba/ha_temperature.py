...
class HATemperaturePlugin(ToolVerba):
    ...
    description = 'Read indoor and outdoor ambient temperature sensors, including weather-station sensors.'
    verba_dec = 'Read Home Assistant ambient temperatures.'
    when_to_use = 'Use for ambient temperature questions, including room-by-room checks.'
    how_to_use = 'Pass a natural-language query asking for current temperature in an area/device.'
    common_needs = ['Room/device temperature target (for example: kitchen temperature).']
    missing_info_prompts = ['Which area or sensor temperature should I check?']
    example_calls = ['{"function":"ha_temperature","arguments":{"query":"what is the temperature in the kitchen"}}', '{"function":"ha_temperature","arguments":{"query":"what is the outside temperature"}}']

    entity_domains: List[str] = ['sensor']
    interpret_actions: List[str] = ['get_state']
    service_map: Dict[str, Optional[str]] = {'get_state': None}
    ...
    interpret_focus: str = '- This verba is read-only.\n- Use get_state only.\n- Match temperature sensors only.'
    interpret_examples: List[str] = ['what is the temperature in the kitchen -> action=get_state, target=kitchen temperature, read_target=state', 'outside temperature right now -> action=get_state, target=outside temperature, read_target=state']

    domain_read_only: bool = True
