...
class HASwitchesPlugin(ToolVerba):
    ...
    when_to_use = 'Use for switch/outlet/plug state checks and on/off control.'
    how_to_use = 'Pass a natural-language request with switch target and desired action.'
    common_needs = ['Switch/device and action (for example: arcade plug + turn on).']
    missing_info_prompts = ['Which switch or plug should I control?']
    example_calls = ['{"function":"ha_switches","arguments":{"query":"turn on the arcade plug"}}', '{"function":"ha_switches","arguments":{"query":"is the desk outlet on"}}']

    entity_domains: List[str] = ['switch']
    interpret_actions: List[str] = ['get_state', 'turn_on', 'turn_off']
    service_map: Dict[str, Optional[str]] = {'get_state': None, 'turn_on': 'turn_on', 'turn_off': 'turn_off'}
    ...
    interpret_focus: str = '- Use turn_on/turn_off for power toggles.\n- Use get_state for status checks.'
    interpret_examples: List[str] = ['turn on arcade plug -> action=turn_on, target=arcade plug, read_target=none', 'is desk outlet on -> action=get_state, target=desk outlet, read_target=state']
