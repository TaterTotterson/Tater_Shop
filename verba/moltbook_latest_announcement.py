...
logger = logging.getLogger("moltbook_latest_announcement")
...
class MoltbookLatestAnnouncementPlugin(ToolVerba):
    name = "moltbook_latest_announcement"
    verba_name = "Moltbook Latest Announcement"
    version = "1.0.2"
    min_tater_version = "59"
    pretty_name = "Moltbook Latest Announcement"
    settings_category = "Moltbook Info"
    tags = ['moltbook', 'latest_announcement']
    fixed_route = "latest_announcement"
    platforms = ['webui', 'macos', 'voice_core', 'homeassistant', 'homekit', 'xbmc', 'discord', 'telegram', 'matrix', 'irc', 'meshtastic']

    usage = (
        "{\"function\":\"moltbook_latest_announcement\",\"arguments\":{\"query\":\"latest Moltbook announcement\"}}"
    )
    description = (
        "Fetch the latest Moltbook announcement from /home."
    )
    verba_dec = "Get the newest Moltbook announcement/news entry."
    when_to_use = (
        "Use when the user asks for latest Moltbook announcement or news."
    )
    how_to_use = (
        "Set `query` to an announcement request (for example: latest Moltbook announcement)."
    )
    common_needs = ['An announcement request in query.']
    routing_keywords = ['moltbook', 'announcement', 'news', 'latest announcement']
    missing_info_prompts = []
    example_calls = ['{"function":"moltbook_latest_announcement","arguments":{"query":"latest Moltbook announcement"}}', '{"function":"moltbook_latest_announcement","arguments":{"query":"any Moltbook news"}}']
    argument_schema = {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'The announcement request (for example: any Moltbook news).'}}, 'required': []}
...
verba = MoltbookLatestAnnouncementPlugin()
