...
class PremiumizeCheckTransferPlugin(PremiumizeBasePlugin):
    ...
    when_to_use = "Use when the request is to check transfer status/progress for a specific Premiumize transfer."
    how_to_use = "Set `query` to a transfer-status request (for example: check transfer 2, status of transfer id abc123)."
    common_needs = ["A transfer-status request in query."]
    ...
    routing_keywords = ["premiumize", "check", "status", "progress", "transfer", "eta"]
    argument_schema = {
        "type": "object",
        "properties": {
            "query": {
