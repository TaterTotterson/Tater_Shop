...
logger = logging.getLogger("moltbook_activity_on_posts")
...
class MoltbookActivityOnPostsPlugin(ToolVerba):
    name = "moltbook_activity_on_posts"
    verba_name = "Moltbook Activity On Posts"
    version = "1.0.1"
    min_tater_version = "59"
    pretty_name = "Moltbook Activity On Posts"
    settings_category = "Moltbook Info"
    tags = ['moltbook', 'activity_on_my_posts']
    fixed_route = "activity_on_my_posts"
    platforms = ['webui', 'macos', 'voice_core', 'homeassistant', 'homekit', 'xbmc', 'discord', 'telegram', 'matrix', 'irc']

    usage = (
        "{\"function\":\"moltbook_activity_on_posts\",\"arguments\":{\"query\":\"any activity on your posts\",\"limit\":5}}"
    )
    description = (
        "Show current activity/replies on your Moltbook posts."
    )
    verba_dec = "Read /home activity buckets for your posts and summarize engagement."
    when_to_use = (
        "Use when the user asks about activity, replies, or engagement on your posts."
    )
    how_to_use = (
        "Set `query` to an activity request and optionally set `limit`."
    )
    common_needs = ['An activity-on-posts request in query.']
    routing_keywords = ['moltbook', 'activity', 'replies', 'engagement', 'comments']
    missing_info_prompts = []
    example_calls = ['{"function":"moltbook_activity_on_posts","arguments":{"query":"any activity on your posts"}}', '{"function":"moltbook_activity_on_posts","arguments":{"query":"show replies on your posts","limit":10}}']
    argument_schema = {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'The activity request (for example: any activity on your posts).'}, 'limit': {'type': 'integer', 'description': 'Optional number of activity entries to return (1-20).'}}, 'required': []}
...
verba = MoltbookActivityOnPostsPlugin()
