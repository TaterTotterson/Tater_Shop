...
logger = logging.getLogger("jackett_indexer_info")
logger.setLevel(logging.INFO)


class JackettIndexerInfoPlugin(ToolVerba):
    name = "jackett_indexer_info"
    verba_name = "Jackett Indexer Info"
    version = "1.0.0"
    min_tater_version = "59"
    pretty_name = "Jackett Indexer Info"
    settings_category = "Jackett"
    tags = ['jackett', 'indexer_info']
    fixed_action = "inspect_indexer"
    usage = (
        '{"function":"jackett_indexer_info","arguments":{"query":"inspect indexer TorrentLeech"}}'
    )
    description = (
        'Inspect one Jackett indexer and return details.'
    )
    verba_dec = 'Show status/privacy/configuration details for a specific Jackett indexer.'
    ...
    when_to_use = (
        'Use when the user asks for details about a specific Jackett indexer.'
    )
    how_to_use = (
        'Set `query` to an inspect request and include the indexer name/id, or pass `indexers`/`indexer` explicitly.'
    )
    common_needs = ['An inspect-indexer request and target indexer name.']
    missing_info_prompts = ['Which Jackett indexer should I inspect?']
    example_calls = ['{"function":"jackett_indexer_info","arguments":{"query":"inspect indexer TorrentLeech"}}', '{"function":"jackett_indexer_info","arguments":{"query":"details for 1337x","indexer":"1337x"}}']
    routing_keywords = ['jackett', 'inspect', 'indexer info', 'details', 'tracker info']
    argument_schema = {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'The inspect request (for example: inspect indexer 1337x).'}, 'indexers': {'type': 'array', 'items': {'type': 'string'}, 'description': 'Optional indexer names/ids to inspect.'}, 'indexer': {'type': 'string', 'description': 'Optional single indexer name/id.'}}, 'required': []}
    ...
    def _attr_map(item: ET.Element) -> Dict[str, str]:
        attrs: Dict[str, str] = {}
        for attr in item.findall("torznab:attr", JackettIndexerInfoPlugin.TORZNAB_NS):
...
verba = JackettIndexerInfoPlugin()
