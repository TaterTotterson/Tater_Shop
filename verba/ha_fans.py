...
class HAFansPlugin(ToolVerba):
    ...
    when_to_use = 'Use for fan status checks and fan on/off/speed control.'
    how_to_use = 'Pass a natural-language query with fan target and requested action.'
    common_needs = ['Fan/device and action (for example: bedroom fan + turn off).']
    missing_info_prompts = ['Which fan should I control?']
    example_calls = ['{"function":"ha_fans","arguments":{"query":"turn on the bedroom fan"}}', '{"function":"ha_fans","arguments":{"query":"set office fan to 40 percent"}}']

    entity_domains: List[str] = ['fan']
    interpret_actions: List[str] = ['get_state', 'turn_on', 'turn_off', 'set_percentage']
    service_map: Dict[str, Optional[str]] = {'get_state': None, 'turn_on': 'turn_on', 'turn_off': 'turn_off', 'set_percentage': 'set_percentage'}
    required_action_params: Dict[str, List[str]] = {'set_percentage': ['percentage']}
    optional_action_params: Dict[str, List[str]] = {}
    param_payload_keys: Dict[str, str] = {}
    summary_attribute_keys: List[str] = ['percentage', 'preset_mode', 'direction']
    interpret_focus: str = '- Use set_percentage for explicit fan speed percentage changes.\n- Use turn_on/turn_off for simple power actions.'
    interpret_examples: List[str] = ['turn on bedroom fan -> action=turn_on, target=bedroom fan, read_target=none', 'set office fan to 40 percent -> action=set_percentage, target=office fan, params.percentage=40, read_target=none', 'is the bedroom fan on -> action=get_state, target=bedroom fan, read_target=state']
