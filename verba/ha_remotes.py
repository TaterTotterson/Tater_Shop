...
class HARemotesPlugin(ToolVerba):
    ...
    verba_dec = 'Control Home Assistant remote entities.'
    when_to_use = 'Use for remote button-style commands and remote power/status checks.'
    how_to_use = 'Pass a natural-language command with remote target and desired button/action.'
    common_needs = ['Remote/device and command (for example: living room remote + mute).']
    missing_info_prompts = ['Which remote should I control?']
    example_calls = ['{"function":"ha_remotes","arguments":{"query":"mute the living room tv"}}', '{"function":"ha_remotes","arguments":{"query":"volume up on the den remote"}}']

    entity_domains: List[str] = ['remote']
    interpret_actions: List[str] = ['get_state', 'turn_on', 'turn_off', 'send_command']
    service_map: Dict[str, Optional[str]] = {'get_state': None, 'turn_on': 'turn_on', 'turn_off': 'turn_off', 'send_command': 'send_command'}
    required_action_params: Dict[str, List[str]] = {'send_command': ['command']}
    optional_action_params: Dict[str, List[str]] = {}
    param_payload_keys: Dict[str, str] = {}
    summary_attribute_keys: List[str] = ['current_activity']
    interpret_focus: str = '- Use send_command for button-like requests (mute, volume, play, pause, home, back, menu, select).\n- Put button command in params.command as a short token.'
    interpret_examples: List[str] = ['mute the living room tv -> action=send_command, target=living room tv, params.command=mute, read_target=none', 'volume up on den remote -> action=send_command, target=den remote, params.command=volume_up, read_target=none', 'is the living room remote on -> action=get_state, target=living room remote, read_target=state']
