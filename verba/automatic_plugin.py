...
class AutomaticPlugin(ToolVerba):
    ...
    verba_dec = "Create one image from a natural-language image request."
    ...
    def _settings(cls) -> Dict[str, str]:
        return cls._decode_map(redis_client.hgetall(f"verba_settings:{cls.settings_category}") or {})
