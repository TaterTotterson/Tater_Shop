...
logger = logging.getLogger("discord_admin_setup")
...
class DiscordAdminSetupPlugin(ToolVerba):
    ...
    usage = (
        '{"function":"discord_admin_setup","arguments":{"request":"Setup this Discord for Tater HQ gamer style"}}'
    )
    description = (
        "Configure Discord server layout and structure: categories, channels, roles, permissions, renames, server name, and optional icon updates."
    )
    when_to_use = (
        "Use for Discord setup/configuration requests in this server, including exact or creative setup modes."
    )
    how_to_use = (
        "Provide one setup request in `request` or `query` (for example: setup this server gamer style, create category Dev with channels bot-dev and tooling)."
    )
    verba_dec = "Discord server setup and structural admin changes."
    argument_schema = {
        "type": "object",
        "properties": {
            "request": {
                "type": "string",
                "description": "Discord setup request (exact or creative).",
            },
            "query": {
                "type": "string",
                "description": "The setup request to run (for example: setup this server gamer style).",
            },
            "dry_run": {
                "type": "boolean",
                "description": "If true, preview the plan without applying changes.",
            },
        },
        "required": [],
    }
    common_needs = [
        "A single natural-language Discord setup request.",
