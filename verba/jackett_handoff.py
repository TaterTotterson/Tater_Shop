...
logger = logging.getLogger("jackett_handoff")
logger.setLevel(logging.INFO)


class JackettHandoffPlugin(ToolVerba):
    name = "jackett_handoff"
    verba_name = "Jackett Handoff"
    version = "1.0.0"
    min_tater_version = "59"
    pretty_name = "Jackett Handoff"
    settings_category = "Jackett"
    tags = ['jackett', 'handoff']
    fixed_action = "handoff"
    usage = (
        '{"function":"jackett_handoff","arguments":{"query":"prepare handoff for result 1 to qBittorrent","target_downloader":"qbittorrent"}}'
    )
    description = (
        'Prepare downloader handoff payload from a selected Jackett result.'
    )
    verba_dec = 'Build transfer payloads (magnet/torrent URI) from cached Jackett search results.'
    ...
    when_to_use = (
        'Use when the user asks to send/pass a selected Jackett result to a downloader.'
    )
    how_to_use = (
        'Set `query` to a handoff request and include result_id/result_title/index. Optionally set target_downloader.'
    )
    common_needs = ['A handoff request and reference to a prior search result.']
    missing_info_prompts = ['Which search result should I prepare for downloader handoff?']
    example_calls = ['{"function":"jackett_handoff","arguments":{"query":"prepare handoff for result 1 to qBittorrent"}}', '{"function":"jackett_handoff","arguments":{"query":"send jkt_abcd1234ef567890 to transmission","result_id":"jkt_abcd1234ef567890","target_downloader":"transmission"}}']
    routing_keywords = ['jackett', 'handoff', 'send to downloader', 'pass to qbittorrent', 'dispatch']
    argument_schema = {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'The handoff request (for example: send result 1 to qBittorrent).'}, 'result_id': {'type': 'string', 'description': 'Optional Jackett result id (jkt_...).'}, 'result_title': {'type': 'string', 'description': 'Optional result title snippet.'}, 'index': {'type': 'integer', 'description': 'Optional 1-based result index from last search.'}, 'target_downloader': {'type': 'string', 'description': 'Optional downloader target label (for example: qbittorrent, transmission).'}}, 'required': []}
    ...
    def _attr_map(item: ET.Element) -> Dict[str, str]:
        attrs: Dict[str, str] = {}
        for attr in item.findall("torznab:attr", JackettHandoffPlugin.TORZNAB_NS):
