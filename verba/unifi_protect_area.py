...
logger = logging.getLogger("unifi_protect_area")
...
# Plugin
# ----------------------------
class UniFiProtectAreaPlugin(ToolVerba):
    """
    ...
    example_calls = ['{"function":"unifi_protect_area","arguments":{"query":"what is going on in the front yard"}}', '{"function":"unifi_protect_area","arguments":{"area":"back yard"}}']

    argument_schema = {'type': 'object', 'properties': {'target': {'type': 'string', 'description': 'Area name to inspect (for example: front yard, back yard, porch, driveway).'}, 'area': {'type': 'string', 'description': 'Area name (alias of target).'}, 'query': {'type': 'string', 'description': 'The area description request (for example: what is going on in the front yard, check the driveway).'}}, 'required': []}
    ...
    def _get_client(self) -> Optional[ProtectClient]:
        try:
            ...
            logger.error(f"[unifi_protect_area] Failed to init client: {e}")
            return None
    ...
    async def _handle(self, args, llm_client, platform: str = "webui", context: Optional[dict] = None):
        ...
        if action in ("sensors_status", "sensor_detail"):
            try:
                ...
                logger.error(f"[unifi_protect_area] list_sensors error: {e}")
                return action_failure(
        ...
        if action == "list_cameras":
            try:
                ...
                logger.error(f"[unifi_protect_area] list_cameras error: {e}")
                return action_failure(
        ...
        if action == "describe_camera":
            ...
            try:
                ...
                logger.error(f"[unifi_protect_area] snapshot error: {e}")
        ...
        if action == "describe_area":
            ...
            for cam in picked:
                ...
                try:
                    ...
                    logger.info(f"[unifi_protect_area] snapshot/vision failed for {cam_name}: {e}")
                    camera_descriptions.append({
                        "camera": cam_name,
                        "state": cam.get("state"),
