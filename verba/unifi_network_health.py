...
load_dotenv()
logger = logging.getLogger("unifi_network_health")
logger.setLevel(logging.INFO)


class UnifiNetworkHealthPlugin(ToolVerba):
    """
    ...
    verba_name = "UniFi Network Health"
    version = "1.0.1"
    min_tater_version = "59"
    pretty_name = "UniFi Network Health"
    tags = ["unifi", "health"]
    fixed_intent = "health"
    description = (
        "Check UniFi Network health and high-level status counts."
    )
    verba_dec = (
        "Get UniFi network health summary including client/device totals and offline device signals."
    )
    when_to_use = (
        "Use for overall network status and health checks."
    )
    how_to_use = (
        "Set query to the health request you want answered (for example: how is the network, any devices offline)."
    )
    example_calls = [
        '{"function":"unifi_network_health","arguments":{"query":"how is the network right now"}}',
        '{"function":"unifi_network_health","arguments":{"query":"network health status"}}',
        '{"function":"unifi_network_health","arguments":{"query":"any devices offline"}}',
    ]
    common_needs = ["Network status question in query."]
    routing_keywords = [
        "unifi",
        "unifi network",
        "network",
    ...
    usage = '{"function":"unifi_network_health","arguments":{"query":"how is the network right now"}}'
    argument_schema = {
        "type": "object",
        "properties": {
            "query": {
                "type": "string",
                "description": "Network health request (for example: how is the network, network status, any devices offline).",
            }
        },
        "required": [],
    }
...
verba = UnifiNetworkHealthPlugin()
