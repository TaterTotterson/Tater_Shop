...
logger = logging.getLogger("shelly_lights")
...
class ShellyLightsPlugin(ToolVerba):
    name = "shelly_lights"
    verba_name = "Shelly Lights"
    pretty_name = "Shelly Lights"
    version = "1.0.0"
    ...
    usage = '{"function":"shelly_lights","arguments":{"query":"set the garage light to 40%"}}'
    description = "Control one light at a time: turn lights on/off, check state, and adjust brightness."
    verba_dec = "Control lights and dimmers."
    when_to_use = (
        "Use for light state checks, power control, and brightness changes."
    )
    how_to_use = "Pass one natural-language light request in query. The verba resolves the light resource and action."
    common_needs = ["Light target and action, such as garage light + set to 40 percent."]
    missing_info_prompts = ["Which light should I control?"]
    example_calls = [
        '{"function":"shelly_lights","arguments":{"query":"turn on the garage light"}}',
        '{"function":"shelly_lights","arguments":{"query":"set the garage light to 40%"}}',
        '{"function":"shelly_lights","arguments":{"query":"is the porch light on?"}}',
    ]
    ...
    async def _ai_interpret(self, llm_client, query: str) -> dict:
        ...
        system = (
            ...
            "Rules:\n"
            "- Use turn_on/turn_off/toggle for light power requests.\n"
            "- Use set_brightness when the user asks for a brightness or dimming percentage.\n"
