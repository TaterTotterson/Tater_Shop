...
class HALocksPlugin(ToolVerba):
    ...
    how_to_use = 'Pass a natural-language lock request with target and action.'
    common_needs = ['Lock/device and action (for example: front door + lock).']
    missing_info_prompts = ['Which lock should I control?']
    example_calls = ['{"function":"ha_locks","arguments":{"query":"lock the front door"}}', '{"function":"ha_locks","arguments":{"query":"is the front door locked"}}']

    entity_domains: List[str] = ['lock']
    interpret_actions: List[str] = ['get_state', 'lock', 'unlock']
    service_map: Dict[str, Optional[str]] = {'get_state': None, 'lock': 'lock', 'unlock': 'unlock'}
    ...
    interpret_focus: str = '- Use lock/unlock for explicit lock control requests.\n- Use get_state for lock status checks.'
    interpret_examples: List[str] = ['lock front door -> action=lock, target=front door, read_target=none', 'unlock back door -> action=unlock, target=back door, read_target=none', 'is the front door locked -> action=get_state, target=front door, read_target=state']
