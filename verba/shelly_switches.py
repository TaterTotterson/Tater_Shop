...
logger = logging.getLogger("shelly_switches")
...
class ShellySwitchesPlugin(ToolVerba):
    name = "shelly_switches"
    verba_name = "Shelly Switches"
    pretty_name = "Shelly Switches"
    version = "1.0.0"
    ...
    tags = ["shelly", "switch", "plug", "outlet", "smart-plug", "relay"]
    routing_keywords = [
        "switch",
        "plug",
        "outlet",
    ...
    usage = '{"function":"shelly_switches","arguments":{"query":"turn on the arcade plug"}}'
    description = "Turn switches, outlets, smart plugs, relays, and plugged-in devices on or off around the house."
    verba_dec = "Control switches, outlets, smart plugs, relays, and plugged-in devices by name."
    when_to_use = (
        "Use for switch, outlet, plug, relay, and plugged-in device state checks and on/off control."
    )
    how_to_use = "Pass one natural-language switch/plug request in query. The verba resolves the Shelly switch resource and action."
    common_needs = ["Switch, plug, outlet, relay, or plugged-in device target plus action."]
    missing_info_prompts = ["Which switch, outlet, plug, relay, or plugged-in device should I control?"]
    example_calls = [
        '{"function":"shelly_switches","arguments":{"query":"turn on the arcade plug"}}',
        '{"function":"shelly_switches","arguments":{"query":"turn off the smart outlet"}}',
        '{"function":"shelly_switches","arguments":{"query":"is the workbench outlet on?"}}',
    ]
    ...
    device_types = {"switch"}
    domain_label = "switch or plug"
    domain_label_plural = "switches and plugs"
    ...
    async def _ai_interpret(self, llm_client, query: str) -> dict:
        ...
        system = (
            ...
            "Rules:\n"
            "- Use turn_on/turn_off/toggle for plugs, switches, outlets, relays, and named plugged-in devices.\n"
            "- Use status for questions asking whether something is on, off, open, closed, or what its state is.\n"
    ...
    async def _ai_choose_device(self, *, query: str, intent: dict, candidates: List[dict], llm_client) -> str:
        ...
        system = (
            "Choose the best device for this request.\n"
