...
class PremiumizeListTransfersPlugin(ToolVerba):
    ...
    description = (
        "List Premiumize transfers with status counts and optional filters."
    )
    verba_dec = (
        "Show Premiumize transfer queue/status with active, finished, and failed counts."
    )
    ...
    when_to_use = (
        "Use when the request is to list transfers or check what Premiumize is downloading."
    )
    how_to_use = (
        "Set `query` to a transfer-list request (for example: show active transfers, list finished Premiumize downloads)."
    )
    common_needs = [
        "A transfer-list request in query.",
    ]
