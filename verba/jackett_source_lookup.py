...
logger = logging.getLogger("jackett_source_lookup")
logger.setLevel(logging.INFO)


class JackettSourceLookupPlugin(ToolVerba):
    name = "jackett_source_lookup"
    verba_name = "Jackett Source Lookup"
    version = "1.0.0"
    min_tater_version = "59"
    pretty_name = "Jackett Source Lookup"
    settings_category = "Jackett"
    tags = ['jackett', 'source_lookup']
    fixed_action = "which_indexer"
    usage = (
        '{"function":"jackett_source_lookup","arguments":{"query":"which indexer was result 2 from"}}'
    )
    description = (
        'Find which Jackett indexer produced a selected prior result.'
    )
    verba_dec = 'Map a cached Jackett result back to its source indexer/tracker.'
    ...
    when_to_use = (
        'Use after a search when the user asks which indexer/tracker a result came from.'
    )
    how_to_use = (
        'Set `query` to a source-lookup request and include result_id/result_title/index when possible.'
    )
    common_needs = ['A source-lookup request and reference to a prior result.']
    missing_info_prompts = ['Which prior result should I map (result id, title, or index)?']
    example_calls = ['{"function":"jackett_source_lookup","arguments":{"query":"which indexer was result 2 from"}}', '{"function":"jackett_source_lookup","arguments":{"query":"source for jkt_abcd1234ef567890","result_id":"jkt_abcd1234ef567890"}}']
    routing_keywords = ['jackett', 'which indexer', 'source', 'tracker source', 'from which tracker']
    argument_schema = {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'The source-lookup request (for example: which indexer was result 2 from).'}, 'result_id': {'type': 'string', 'description': 'Optional Jackett result id (jkt_...).'}, 'result_title': {'type': 'string', 'description': 'Optional result title snippet.'}, 'index': {'type': 'integer', 'description': 'Optional 1-based result index from last search.'}}, 'required': []}
    ...
    def _attr_map(item: ET.Element) -> Dict[str, str]:
        attrs: Dict[str, str] = {}
        for attr in item.findall("torznab:attr", JackettSourceLookupPlugin.TORZNAB_NS):
...
verba = JackettSourceLookupPlugin()
