...
logger = logging.getLogger("moltbook_agent_profile")
...
class MoltbookAgentProfilePlugin(ToolVerba):
    name = "moltbook_agent_profile"
    verba_name = "Moltbook Agent Profile"
    version = "1.0.1"
    min_tater_version = "59"
    pretty_name = "Moltbook Agent Profile"
    settings_category = "Moltbook Info"
    tags = ['moltbook', 'agent_profile']
    fixed_route = "agent_profile"
    platforms = ['webui', 'macos', 'voice_core', 'homeassistant', 'homekit', 'xbmc', 'discord', 'telegram', 'matrix', 'irc']

    usage = (
        "{\"function\":\"moltbook_agent_profile\",\"arguments\":{\"query\":\"show profile for clawdclawderberg\",\"target_name\":\"clawdclawderberg\"}}"
    )
    description = (
        "Fetch another Moltbook agent profile by name."
    )
    verba_dec = "Lookup target Moltbook agent profile details."
    when_to_use = (
        "Use when the user asks for another agent profile on Moltbook."
    )
    how_to_use = (
        "Set `query` to a profile lookup and include the target name (or pass `target_name`)."
    )
    common_needs = ['An agent-profile request and target agent name.']
    routing_keywords = ['moltbook', 'agent profile', 'profile for', 'show profile']
    missing_info_prompts = []
    example_calls = ['{"function":"moltbook_agent_profile","arguments":{"query":"show profile for clawdclawderberg"}}', '{"function":"moltbook_agent_profile","arguments":{"query":"agent profile for botalpha","target_name":"botalpha"}}']
    argument_schema = {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'The agent-profile request (for example: show profile for <name>).'}, 'target_name': {'type': 'string', 'description': 'Target Moltbook agent name to look up.'}}, 'required': []}
...
verba = MoltbookAgentProfilePlugin()
