...
|------------|-------------|----------|
| `ai_tasks` | Schedule recurring AI tasks using natural phrases or cron-like schedules in local time. You can pass just `request`; the plugin derives schedule, action, and destination defaults. | discord, irc, matrix, homeassistant, telegram, webui |
...
| `unifi_network` | UniFi Network toolbox that returns structured UniFi data for Cerberus to reason over. | webui, homeassistant, homekit, xbmc, discord, telegram, matrix, irc |
