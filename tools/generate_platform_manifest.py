...
def humanize_platform_id(platform_id: str) -> str:
    parts = [p for p in str(platform_id or "").split("_") if p]
    if not parts:
        return str(platform_id or "platform")
...
def extract_platform_settings(tree: ast.AST, constants: dict[str, object]) -> dict[str, object]:
    for node in getattr(tree, "body", []):
        if not (isinstance(node, ast.Assign) and len(node.targets) == 1 and isinstance(node.targets[0], ast.Name)):
            continue
        if node.targets[0].id != "PLATFORM_SETTINGS":
            continue
...
def extract_platform_meta(py_file: Path, min_tater_version_default: str) -> dict[str, object]:
    source = py_file.read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(py_file))
    constants = collect_simple_constants(tree)
    settings_meta = extract_platform_settings(tree, constants)

    module_key = py_file.stem
    platform_id = module_key[:-len("_platform")] if module_key.endswith("_platform") else module_key

    category = str(settings_meta.get("category") or "").strip()
    display_name = category.replace(" Settings", "").strip() if category else humanize_platform_id(platform_id)

    doc = ast.get_docstring(tree) or ""
    first_doc_line = next((line.strip() for line in doc.splitlines() if line.strip()), "")

    description = str(
        constants.get("PLATFORM_DESCRIPTION")
        or constants.get("DESCRIPTION")
        or first_doc_line
        or (f"{display_name} integration platform for Tater.")
    ).strip()
...
def build_manifest(platforms_dir: Path, root: Path, min_tater_version_default: str) -> tuple[dict, list[dict[str, str]]]:
    if not platforms_dir.exists():
        raise FileNotFoundError(f"Missing platforms dir: {platforms_dir}")

    items = []
    errors = []

    for py_file in sorted(platforms_dir.glob("*_platform.py")):
        ...
        try:
            meta = extract_platform_meta(py_file, min_tater_version_default=min_tater_version_default)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Generate platform manifest for Tater Shop")
    parser.add_argument("--platforms-dir", default=str(root / "platforms"))
    parser.add_argument("--output", default=str(root / "platform_manifest.json"))
    parser.add_argument("--min-tater-version", default="59")
    args = parser.parse_args()

    platforms_dir = Path(args.platforms_dir).resolve()
    ...
    if errors:
        print("Platform manifest build errors:")
        for err in errors:
            print(f" - {err['file']}: {err['error']}")
        return 1

    print(f"Wrote {output_path} with {len(manifest['platforms'])} platforms")
    return 0
