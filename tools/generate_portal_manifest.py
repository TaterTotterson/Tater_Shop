...
def extract_portal_settings(tree: ast.AST, constants: dict[str, object]) -> dict[str, object]:
    for node in getattr(tree, "body", []):
        if not (isinstance(node, ast.Assign) and len(node.targets) == 1 and isinstance(node.targets[0], ast.Name)):
            continue
        if node.targets[0].id != "PORTAL_SETTINGS":
            continue
...
def extract_portal_meta(py_file: Path, min_tater_version_default: str) -> dict[str, object]:
    source = py_file.read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(py_file))
    constants = collect_simple_constants(tree)
    settings_meta = extract_portal_settings(tree, constants)

    module_key = py_file.stem
    portal_id = module_key[:-len("_portal")] if module_key.endswith("_portal") else module_key

    category = str(settings_meta.get("category") or "").strip()
    display_name = category.replace(" Settings", "").strip() if category else portal_id.replace("_", " ").title()

    doc = ast.get_docstring(tree) or ""
    first_doc_line = next((line.strip() for line in doc.splitlines() if line.strip()), "")

    description = str(
        constants.get("PORTAL_DESCRIPTION")
        or constants.get("DESCRIPTION")
        or first_doc_line
        or (f"{display_name} integration portal for Tater.")
    ).strip()
...
def build_manifest(portals_dir: Path, root: Path, min_tater_version_default: str) -> tuple[dict, list[dict[str, str]]]:
    if not portals_dir.exists():
        raise FileNotFoundError(f"Missing portals dir: {portals_dir}")

    items = []
    errors = []

    for py_file in sorted(portals_dir.glob("*_portal.py")):
        if py_file.name.startswith("_"):
            continue
        rel_entry = str(py_file.relative_to(root)).replace("\\", "/")
        try:
            meta = extract_portal_meta(py_file, min_tater_version_default=min_tater_version_default)
    ...
    return {"schema": SCHEMA_VERSION, "portals": items}, errors
...
def main() -> int:
    ...
    if errors:
        print("Portal manifest build errors:")
        for err in errors:
            print(f" - {err['file']}: {err['error']}")
        return 1

    print(f"Wrote {output_path} with {len(manifest['portals'])} portals")
    return 0
